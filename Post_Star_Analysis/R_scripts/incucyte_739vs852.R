...
colData <- read.csv("./Data/ColData_3_Incucyte_739vs852.csv", row.names = 1)
...
# 5. Write to CSV
write.csv(resDF, file = "./incucyte_739vs852_DEgenes.csv")

resSig <- resDF[resDF$Significant == "Yes", ]
write.csv(resSig, file = "./incucyte_739vs852_DEGs_only.csv")
################################################################################
...
data <- read.csv("./incucyte_739vs852_DEGs_only.csv", row.names = 1)
...
write.csv(x = gse_df_filtered, file = "./gse_analysis_739vs852.csv")
...
ggsave("./gse_dotplot_739vs852.jpg", plot = p, width = 10, height = 7, dpi = 600)
